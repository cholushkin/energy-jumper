...
// PowerSupplyAndSwitcher
public class PowerSupplyAndSwitcherDischargeSurface : MonoBehaviour
{
    ...
    private void Init()
    {
        ...
        _nonActiveMaterial = GamePrefabs.Instance.Materials["YellowBright"];
        Assert.IsTrue(HasPowerEffect(), "you have attach DischargeSurfaceCooldown only to node with NodeProps.DischargeSurface");
    ...
    {
        return _node.NodeProps.HasFlag(NodeProps.DischargeSurface);
    }
    ...
    public void ToggleSwitch(bool activeState, bool ignoreCooldown = false) 
    {
        ...
        if (_activeState)
        {
            ...
            _node.NodeProps |= NodeProps.DischargeSurface;
    ...
    void DbgSeetPowerTrue()
    {
        SetPower(true);
    }

    [ContextMenu("SetPower(false)")]
    void DbgSeetPowerFalse()
    {
        SetPower(false);
    }
}
